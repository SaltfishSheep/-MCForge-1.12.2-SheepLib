...
public class PacketSender {
    ...
    private static final String NAME = "SHEEPPACKET";
    ...
    public void registerPacket(String registerName, PacketBuilder packetBuilder){
        registry.put(registerName,packetBuilder);
    }
    ...
    public void sendToDim(PacketBase packet, int dim) {
        try {
            CHANNEL.sendToDimension(new FMLProxyPacket(write(packet), NAME), dim);
    ...
    public void sendAroundPos(PacketBase packet, int dim, double x, double y, double z, double range) {
        try {
            CHANNEL.sendToAllAround(new FMLProxyPacket(write(packet), NAME), new TargetPoint(dim, x, y, z, range));
        } catch (Throwable error) {
            SheepLib.printError(error);
        }
    }
    ...
    public void sendToPlayer(PacketBase packet, EntityPlayerMP player) {
        try {
            CHANNEL.sendTo(new FMLProxyPacket(write(packet), NAME), player);
        } catch (Throwable error) {
            SheepLib.printError(error);
        }
    }

    public void sendToPlayers(PacketBase packet, List<EntityPlayerMP> players) {
        try {
    ...
    public void sendToAll(PacketBase packet) {
        try {
            CHANNEL.sendToAll(new FMLProxyPacket(write(packet), NAME));
        } catch (Throwable error) {
            SheepLib.printError(error);
        }
    }
    ...
    public void sendToServer(PacketBase packet) {
        try {
            CHANNEL.sendToServer(new FMLProxyPacket(write(packet), NAME));
        } catch (Throwable error) {
            SheepLib.printError(error);
        }
    }
    ...
    private void receive(FMLProxyPacket fmlP, boolean isRemote) throws InstantiationException, IllegalAccessException, IOException {
        ...
        PacketBase packet = registry.get(registerName).build();
        packet.readBuf(buf);
        if (isRemote)
            Minecraft.getMinecraft().addScheduledTask(packet::process);
        else
            SheepLib.getMCServer().addScheduledTask(packet::process);
    }
    ...
    public static class PacketBuilderClass implements PacketBuilder{
        Class<? extends PacketBase> clazz;
        public PacketBuilderClass(Class<? extends PacketBase> clazz){this.clazz=clazz;}
        @Override
